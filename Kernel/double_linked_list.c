...
int removeElement(DoubleLinkedListADT list, void *data) {
    ...
    while (it) {
        if (it->data == data) {
            if (it->prev) it->prev->next = it->next; else list->first = it->next;
            if (it->next) it->next->prev = it->prev; else list->last = it->prev;
            freeMemory(it);
            list->size--;
            return 0;
        }
